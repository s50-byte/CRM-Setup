...
export default function Gantt() {
    ...
    const monate = [];
    {
        ...
        while (cur < bisDate) {
            ...
            monate.push({ label: `${MONATE[cur.getMonth()]} ${String(cur.getFullYear()).slice(2)}`, days });
            cur = next;
        }
    }
            ...
            <div style={{ ...CARD, overflow: 'hidden' }}>
                ...
                {!laden && !fehler && daten.length > 0 && (
                    <div style={{ display: 'flex' }}>
                        {/* LINKE SPALTE */}
                        <div style={{ width: LEFT_W, flexShrink: 0, borderRight: '1px solid rgba(0,0,0,.09)' }}>
                            <div style={{
                        ...
                        <div style={{ flex: 1, overflowX: 'auto' }}>
                            <div style={{ position: 'relative', minWidth: 720 }}>
                                ...
                                <div style={{ display: 'flex', height: HEADER_H, borderBottom: '1px solid rgba(0,0,0,.09)', background: '#F5F4F0', position: 'relative', zIndex: 1 }}>
                                    {monate.map((m, i) => (
                                        <div key={i} style={{
                                            flex: m.days, display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            fontSize: 10.5, fontWeight: 600, color: '#6B6860',
                                            borderRight: i < monate.length - 1 ? '1px solid rgba(0,0,0,.06)' : 'none',
