...
router.post('/', auth, async (req, res) => {
    const { klient_id, datum, status, ankunftszeit, bemerkung, kommentar, automatisch } = req.body;
    ...
    try {
        ...
        if (statusGeaendert || kommentarGeaendert) {
            ...
            // Gemeldet wird nur, wenn eine Person den Status wirklich gesetzt hat.
            // Die Praesenzseite legt beim Oeffnen fuer jeden Klienten ohne Eintrag
            // automatisch "anwesend" an – solche Eintraege tragen automatisch:true
            // und loesen nichts aus (Feedback 23.06.2026).
            //
            // Empfaenger ist die Klientenfuehrung; ist keine gesetzt, ersatzweise die
            // uebrigen aktiven Zuweisungen, damit niemand leer ausgeht. Wer selbst
            // erfasst, wird NICHT ausgenommen – die Klientenfuehrung will den Eintrag
            // auch dann auf dem Dashboard sehen (Feedback 31.08.2026).
            const kaderResult = automatisch
                ? { rows: [] }
                : await db.query(
