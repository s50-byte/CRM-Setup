...
-- Alle abhängigen Daten löschen (wegen NOT NULL / FK Constraints)
DELETE FROM dossier_phase;
DELETE FROM vereinbarungsziel;
DELETE FROM kriterium_status;
DELETE FROM verfuegung_position;
DELETE FROM verfuegung;
DELETE FROM praesenz_eintrag;
DELETE FROM journal_eintrag;
DELETE FROM zeitachse_eintrag;
DELETE FROM task;
DELETE FROM termin_user;
DELETE FROM termin;
DELETE FROM klient_user;
DELETE FROM programm_verlauf;

-- Dossier-Referenzen auf Programme/Phasen nullen
UPDATE dossier SET akt_programm_id = NULL, akt_phase_id = NULL;

-- Alte Programme löschen (Phasen/Kriterien via CASCADE, akt_phase_id bereits NULL)
...
-- FK-abhängige Leistungen löschen (verfuegung_position bereits gelöscht)
DELETE FROM leistung WHERE tarifnr LIKE '1%';
DELETE FROM leistung WHERE tarifnr = '9999';

-- Gruppe BM: Berufliche Massnahmen
INSERT INTO programm (programm_id, name, farbe_hex, gruppe, leistung_id, tarif_pro_tag)
